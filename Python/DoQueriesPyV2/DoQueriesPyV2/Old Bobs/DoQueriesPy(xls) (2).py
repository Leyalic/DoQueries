...
proff_akr_mail =send_to(amy, karen, ryan, proff)
...
def do_dailies():
    ...
    for query in os.listdir("."):
        ...
        if query.startswith("UUFA_IL_UPDATED_ATB_" + year):
...
def do_monday_weeklies():
    ...
    for query in os.listdir("."):
        ...
        if query.startswith("FA_WR_SUMMER_NO_DL_13"):
            do_query(query, date + " Summer Enroll No DL " + year + ".xls", directory2013,
                     ak_attachment_list)
            n
        if query.startswith("FA_WR_FALL_TOTAL_WDRN_DRP_14"):
            do_query(query, date + " Fall Total Withdrawn Drop 14.xls", directory2014,
                     akr_attachment_list)

        if query.startswith("FA_WR_SPR_TOTAL_WDRN_DRP_14"):
            do_query(query, date + " Spring Total Withdrawn Drop 14.xls", directory2014,
                     akr_attachment_list)

        if query.startswith("FA_WR_SUM_TOTAL_WDRN_DRP_14"):
    ...
    if proff_attachment_list:
        mailer("", aid_year + " Monday Weekly Queries", proff_mail, "", proff_attachment_list)
        del proff_attachment_list[:]
...
def do_budget_queries():
    ...
    if proff_attachment_list:
        mailer("", aid_year + " Budget Queries", proff_mail, "", proff_attachment_list)
        del proff_attachment_list[:] 
...
def do_packaging_queries():
    ...
    # will be added.
    for query in os.listdir("."):
        if query.startswith("UUFA_PRT_ACAD_LVLS_OUT_OF_SYNC"):
        ...
        if query.startswith("UUFA_PRT_AWD_SUB_OVER_AGG_RVW"):
            do_query(query, date + " SUB Over Aggregate.xls", directory,
                     loans_a_attachment_list)

        if query.startswith("UUFA_PRT_CTZN_IND_AWD_NO_LOANS"):
        ...
        if query.startswith("UUFA_PRT_DEPEND_PRNT_SSN_RVW"):
        ...
        if query.startswith("UUFA_PRT_GRANT_UG_5TH_YR_2BACH"):
        ...
        if query.startswith("UUFA_PRT_LOAN_CBA_AWD_NOT_ELIG"):
    ...
    if ms_attachment_list:
        ...
        del ms_attachment_list[:]
    if proff_attachment_list:
        mailer("", aid_year + " Batch Packaging Queries", proff_mail, "", proff_attachment_list)
        del proff_attachment_list[:]
...
def do_monthlies():
    global aid_year
    for query_name in os.listdir("."):
        if query_name.startswith("UUFA_MR_ATHLETE_RESIDENCY_"):
            year = str(int(re.search(r'\d+', query_name).group()))
            break
    aid_year = "20" + str(int(year) - 1) + "-20" + year
    # Create FOLDER variables to be used in Move() operation and establishes the directory to save the files
    ...
    for query in os.listdir("."):
        ...
        if query.startswith("UUFA_MR_UNDS_OFFER_SCHOLAR_" + year):
            do_query(query, date + " Scholarship Awards UNDS Career " + year + ".xls", directory,
                     ac_attachment_list)

        if query.startswith("UUFA_MR_UNDS_OFRD_AMT_ATH_" + year):
            do_query(query, date + " Athlete Awards UNDS Career " + year + ".xls", directory,
                     athletics_attachment_list)

        if query.startswith("UUFA_MR_UNDS_OFRD_AMT_FDRL_" + year):
            do_query(query, date + " Federal Awards UNDS Career " + year + ".xls", directory,
                     akr_attachment_list)

        if query.startswith("UUFA_MR_VERIFY_DEP_OVERRIDE_" + year):
    ...
    if loans_ak_attachment_list:
        ...
        del loans_ak_attachment_list[:]
    if proff_attachment_list:
        mailer("", aid_year + " Monthly Queries", proff_mail, "", proff_attachment_list)
        del proff_attachment_list[:]
    if rac_attachment_list:
...
def do_end_of_term_queries():
    ...
    for query in os.listdir("."):
        ...
        if query.startswith("UUFA_MR_FWS_WITH_NSI_HOLD_" + year):
    ...
    if loans_a_attachment_list:
        ...
        del loans_a_attachment_list[:]
    if proff_attachment_list:
        mailer("", "End of Term Queries", proff_mail, "", proff_attachment_list)
        del proff_attachment_list[:]
    if akr_attachment_list:
...
for filename in os.listdir("."):
    ...
    # Monthly Queries
    if filename.startswith("UUFA_MR_ATHLETE_RESIDENCY_"):
        do_monthlies()
