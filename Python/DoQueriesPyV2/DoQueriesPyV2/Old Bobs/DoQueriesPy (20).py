...
def do_monday_weeklies():
    ...
    if test:
        ...
        directory2013 = os.path.realpath(os.path.join('C:\Testing Bob\Monday Weekly', "2012-2013", month_folder))
    ...
    for query in os.listdir("."):
        ...
        if query.startswith("FA_WR_FALL_TOTAL_WTHDRN_DRP_" + year) | query.startswith("UUFA_WR_FALL_TOTAL_WDRN_DRP_" + year):
        ...
        if query.startswith("FA_WR_SPR_TOTAL_WDRN_DRP_" + str(int(year)-1)) | query.startswith("UUFA_WR_SPR_TOTAL_WDRN_DRP_" + year):
        ...
        if query.startswith("FA_WR_SUM_TOTAL_WDRN_DRP_" + str(int(year)-1)) | query.startswith("UUFA_WR_SUM_TOTAL_WDRN_DRP_" + year):
        ...
        if query.startswith("FA_WR_SUSP_DOB_PRB_APPLCNT_" + year) | query.startswith("UUFA_WR_SSP_DOB_PRB_APPLCNT_" + year):
            do_query(query, date + " Suspense Applicant DOB Problem 20" + year + ".xlsx", directory,
                     akr_attachment_list)

        if query.startswith("FA_WR_SUSP_NAME_PRB_APPLCNT_" + year) | query.startswith("UUFA_WR_SSP_NAME_PRB_APLCNT_" + year):
            do_query(query, date + " Suspense Applicant Name Problem 20" + year + ".xlsx", directory,
                     akr_attachment_list)

        if query.startswith("UUFA_WR_SSP_SSN_PRB_APLCNT_" + year) | query.startswith("UUFA_WR_SSP_SSN_PRB_APLCNT_20" + year):
        ...
        if query.startswith("FA_WR_NSL_PROMISSORY_NOTE") | query.startswith("UUFA_WR_NSL_PROMISSORY_"):
...
def do_budget_test_queries():
    global aid_year
    year = str(int(date[-2:]) + 1)
    for query_name in os.listdir("."):
        if query_name.startswith("UUFA_BUDGET_20" + year + "_DN1"):
            if date[6:] in query_name:
                aid_year = "20" + str(int(query_name[-22:-20]) - 1) + "-20" + query_name[-22:-20]
                break
            if str(int(date[6:]) + 1) in query_name:
                aid_year = "20" + str(int(query_name[-22:-20]) - 1) + "-20" + query_name[-22:-20]
                break
...
def do_weekly_scholarships():
    global aid_year
    for query_name in os.listdir("."):
        if query_name.startswith("UUFA_BOOKS_NOPOST_")|query_name.startswith("UUFA_SCH_ALL_NEED_"):
            if date[6:] in query_name:
                aid_year = "20" + str(int(query_name[-15:-13]) - 1) + "-20" + query_name[-15:-13]
                break
            if str(int(date[6:]) + 1) in query_name:
...
for filename in os.listdir("."):
    ...
    if filename.startswith("UUFA_BOOKS_NOPOST_")|filename.startswith("UUFA_SCH_ALL_NEED_"):
