...
def rename(name, new_name, attach_list, i=2):
    ...
    try:
        ...
        try:
            final_name = this_new_name[:-4] + " (" + str(num) + ").xls"
...
def do_monday_weeklies():
    ...
    for query in os.listdir("."):
        ...
        if query.startswith("FA_WR_FALL_TOTAL_WDRN_DRP_" + str(int(year)-1)):
            do_query(query, date + " Fall Disb Total Withdrawn Drop " + str(int(year)-1) + " .xls", directory,
                     akr_attachment_list)

        if query.startswith("FA_WR_SPR_TOTAL_WDRN_DRP_" + str(int(year)-1)):
            do_query(query, date + " Spring Disb Total Withdrawn Drop " + str(int(year)-1) + " .xls", directory,
                     akr_attachment_list)

        if query.startswith("FA_WR_SUM_TOTAL_WDRN_DRP_" + str(int(year)-1)):
            do_query(query, date + " Summer Disb Total Withdrawn Drop " + str(int(year)-1) + " .xls", directory,
                     akr_attachment_list)
...
def do_end_of_term_queries():
    ...
    for query in os.listdir("."):
        ...
        if query.startswith("UUFA_SCHOLAR_ACAD_PROG_REVIEW"):
            do_query(query, date + " Scholarship Academic Progress Review.xls", directory,
                     act_attachment_list)
