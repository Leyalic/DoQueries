...
js_mail =       send_to(jonathan, sheryl)
rk_mail =       send_to(ryan, karen)
...
rkl_mail =      send_to(ryan, karen, linh)
...
laura_mail =    send_to(laura)
leo_mail =      send_to(leo)
loans_rk_mail = send_to(loans, ryan, karen)
...
meb_mail =      send_to(mary, emilie, brenda)
ms_mail =       send_to(mat, scott)
mary_s_mail =   send_to(mary, shanon)
null_mail =     ""
prof_mail =     send_to(prof, ryan)
prof_k_mail =   send_to(prof, karen)
prof_rk_mail =  send_to(prof, ryan, karen)
...
schol_mail =    send_to(raenetta,jonathan, sheryl, plooster )
...
laura_attachment_list = []
leo_attachment_list = []
loans_r_attachment_list = []
loans_rk_attachment_list = []
...
prof_k_attachment_list = []
prof_rk_attachment_list = []
rac_attachment_list = []
...
def do_dailies():
    ...
    if rk_attachment_list:
        mailer("", aid_year + " Daily Queries", rk_mail, "", rk_attachment_list)
        del rk_attachment_list[:]
...
def do_monday_weeklies():
    ...
    if js_attachment_list:
        mailer("", aid_year + " Monday Weekly Queries", js_mail, "", js_attachment_list)
        del js_attachment_list[:]
    if rk_attachment_list:
        mailer("", aid_year + " Monday Weekly Queries", rk_mail, "", rk_attachment_list)
        del rk_attachment_list[:]
    if rkm_attachment_list:
    ...
    if loans_r_attachment_list:
        ...
        del loans_r_attachment_list[:]
    if loans_rk_attachment_list:
        mailer("", aid_year + " Monday Weekly Queries", loans_rk_mail, "", loans_rk_attachment_list)
        del loans_rk_attachment_list[:]
    if mat_attachment_list:
...
def do_budget_queries():
    ...
    if rk_attachment_list:
        mailer("", aid_year + " Budget Queries", rk_mail, "", rk_attachment_list)
        del rk_attachment_list[:]
...
def do_packaging_queries():
    ...
    if rk_attachment_list:
        mailer("", aid_year + " Batch Packaging Queries", rk_mail, "", rk_attachment_list)
        del rk_attachment_list[:]
    if rkm_attachment_list:
    ...
    if loans_r_attachment_list:
        ...
        del loans_r_attachment_list[:]
    if loans_rk_attachment_list:
        mailer("", aid_year + " Batch Packaging Queries", loans_rk_mail, "", loans_rk_attachment_list)
        del loans_rk_attachment_list[:]
...
def do_monthlies():
    ...
    if aka_attachment_list:
        ...
        del aka_attachment_list[:]
    if js_attachment_list:
        mailer("", aid_year + " Monthly Queries", js_mail, "", js_attachment_list)
        del js_attachment_list[:]
    ...
    if loans_r_attachment_list:
        ...
        del loans_r_attachment_list[:]
    if loans_rk_attachment_list:
        mailer("", aid_year + " Monthly Queries", loans_rk_mail, "", loans_rk_attachment_list)
        del loans_rk_attachment_list[:]
...
def do_end_of_term_queries():
    ...
    for query in os.listdir("."):
        ...
        if query.startswith("UUFA_EOT_SCH_ACD_PRG_REVIEW_79"):
    ...
    if rk_attachment_list:
        mailer("", "End of Term Queries", rk_mail, "", rk_attachment_list)
        del rk_attachment_list[:]
    if rkjm_attachment_list:
        mailer("", "End of Term Queries", rkjm_mail, "", rkjm_attachment_list)
        del rkjm_attachment_list[:]
    if rkl_attachment_list:
        mailer("", "End of Term Queries", rkl_mail, "", rkl_attachment_list)
        del rkl_attachment_list[:]
    if loans_rk_attachment_list:
        mailer("", "End of Term Queries", loans_rk_mail, "", loans_rk_attachment_list)
        del loans_rk_attachment_list[:]
    if loans_r_attachment_list:
        mailer("", "End of Term Queries", loans_r_mail, "", loans_r_attachment_list)
        del loans_r_attachment_list[:]
    if prof_k_attachment_list:
        mailer("", "End of Term Queries", prof_k_mail, "", prof_k_attachment_list)
        del prof_k_attachment_list[:]
    if meb_attachment_list:
        mailer("", "End of Term Queries", meb_mail, "", meb_attachment_list)
        del meb_attachment_list[:]
    if prof_rk_attachment_list:
        mailer("", "End of Term Queries", prof_rk_mail, "", prof_rk_attachment_list)
        del prof_rk_attachment_list[:]
    if js_attachment_list:
        mailer("", "End of Term Queries", js_mail, "", js_attachment_list)
        del js_attachment_list[:]
...
def do_2nd_ldr():
    ...
    if ac_attachment_list:
        ...
        del ac_attachment_list[:]
    if rk_attachment_list:
        mailer("", "Second Session LDR Queries", rk_mail, "", rk_attachment_list)
        del rk_attachment_list[:]
    if rkm_attachment_list:
